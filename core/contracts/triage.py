...
class TaskSpec(BaseModel):
    problem: str = Field(
        ..., description="A clear and concise description of the problem to be solved."
    )
    ...
    out_of_scope: List[str] = Field(
        ..., description="List of items that are explicitly out of scope for this task."
    )
...
class AmbiguityResult(BaseModel):
    status: StageStatus = Field(
        ...,
        description="Indicates whether the task specification is clear (ok) or if it requires human intervention due to ambiguities.",
    )
    questions: List[str] = Field(
        default_factory=list,
        description="List of questions that highlight potential ambiguities in the task specification.",
    )
...
class TriageResult(BaseModel):
    ...
    questions: List[str] = Field(
        default_factory=list, description="List of questions to be asked during the triage process."
    )
