...
def test_triage_step_blocks_when_issue_title_missing(monkeypatch) -> None:
    monkeypatch.setattr(triage_steps, "get_issue_details", lambda **kwargs: {"title": "", "body": ""})
    step = TriageStep()
    run = RunModel(state="RECEIVED", repository="acme/repo", issue_number=5)
    result = step.execute({"repository": "acme/repo", "issue_number": 5}, run, _FakeRuntime(StageResult(stage="x")))
...
def test_code_step_normalizes_inputs_and_wraps_worker_output(monkeypatch) -> None:
    ...
    worker_output = {"files_map": {"app/a.py": "print('x')"}, "tests_map": {"tests/test_a.py": "def t(): pass"}}

    monkeypatch.setattr(coding_steps, "CodeWorker", _FakeWorker)
    monkeypatch.setattr(coding_steps.ray, "get", lambda refs: [(StageStatus.OK, worker_output) for _ in refs])
    step = CodeStep()
    run = RunModel(state="PLANNED")
    runtime = _FakeRuntime(StageResult(stage=PipelineStage.CODE, status=StageStatus.OK))
    context = {
        "steps": [{"title": "Do work", "objective": "Patch code", "files": ["app/a.py"], "tests": ["tests/test_a.py::test_x"]}],
        "repo_map": 123,
...
def test_qa_step_blocks_when_coding_payload_is_invalid() -> None:
    ...
    result = step.execute({"coding_output": {}, "coding_step": {}}, run, _FakeRuntime(StageResult(stage="x")))
    assert result.status == StageStatus.BLOCKED
    assert "invalid coding payload" in result.notes["reason"]

def test_pr_open_step_respects_policy_block(monkeypatch) -> None:
    monkeypatch.setattr(pr_open_steps, "can_open_pr", lambda qa_result: PRDecision(allowed=False, reason="qa blocked", blocking_reasons=["qa_not_green"]))
    step = PROpenStep()
    run = RunModel(state="QA_RUNNING", issue_number=99, repository="acme/repo")
    context = {"_stage_results": {PipelineStage.QA.value: StageResult(stage=PipelineStage.QA.value, status=StageStatus.BLOCKED)}}
...
def test_review_step_after_sets_blocked_when_policy_disallows() -> None:
    ...
    transitions = step.after(
        result=result,
        context={"policy_decision": {"allowed": False, "reason": "manual block", "blocking_reasons": ["risk_high"]}},
        run=run,
    )
    assert transitions == []
    assert result.status == StageStatus.BLOCKED
...
def test_merge_step_dispatches_worker_and_maps_outputs(monkeypatch) -> None:
    ...
    step = MergeStep()
    run = RunModel(state="READY_TO_MERGE", repository="acme/repo", pull_request_number=12, metadata={"m": 1})
...
def test_publish_step_dispatches_worker_and_maps_outputs(monkeypatch) -> None:
    ...
    step = PublishStep()
    run = RunModel(state="QA_RUNNING", repository="acme/repo", issue_number=7, metadata={"origin": "test"})
    runtime = _FakeRuntime(
        StageResult(
            stage=PipelineStage.PUBLISH,
            status=StageStatus.OK,
            outputs={
                "outputs": {"publish_output": "Published changes to autopr/issue-7.", "head_branch": "autopr/issue-7"},
